...
module Test.DictaTask.Parse.DedupSpec (spec) where

import Test.Hspec
import DictaTask.Parse.Dedup
...
spec = do
  ...
  describe "checkDuplicate" $ do
    it "detects exact duplicates" $ do
      let existing = [("id-1", "Fix the build", Just "dicta-task")]
      let result = checkDuplicate "Fix the build" (Just "dicta-task") existing
