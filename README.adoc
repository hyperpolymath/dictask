...
*dictask* automates the ingestion of speech notes from a portable voice
...
|===
|Directory |Purpose
|`+.github/+` |Forge-specific metadata (CODEOWNERS, SECURITY.md,
workflows).

|`+.machine_readable/+` |Canonical project state (6 a2ml files), bot
directives, and AI guides.

|`+src/ingest/+` |Rust crate: udev detection, file archival, cloud
upload.
...
|`+src/interface/+` |Verified Interface Seams (Idris2 ABI, Zig FFI,
generated C headers).

|`+deploy/ansible/+` |Ansible playbooks for local machine setup.

|`+deploy/terraform/+` |Terraform configs for cloud provisioning
(bucket, IAM).
