...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "dicta-task"`
...
# an instantiation. The template submitted the literal `dicta-task` as its own
...
# is set to `owner/dicta-task` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `dicta-task-2`,
