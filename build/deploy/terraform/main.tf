...
#
# Terraform configuration for dicta-task cloud backup.
...
variable "bucket_name" {
  ...
  type        = string
  default     = "dicta-task-audio-backup"
}
...
resource "google_service_account" "dicta_task_uploader" {
  account_id   = "dicta-task-uploader"
  display_name = "dicta-task Audio Uploader"
  description  = "Service account for dicta-task pipeline to upload encrypted audio backups"
}
...
resource "google_storage_bucket_iam_member" "uploader_create" {
  ...
  member = "serviceAccount:${google_service_account.dicta_task_uploader.email}"
}
...
output "service_account_email" {
  ...
  value       = google_service_account.dicta_task_uploader.email
}
