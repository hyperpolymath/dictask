...
== DICTA_TASK ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libdicta_task.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── dicta_task.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── dicta_task.h       # Generated from Idris2 ABI
│
...
----
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/dicta_task.h
----
...
[source,c]
----
#include "dicta_task.h"

int main() {
    void* handle = dicta_task_init();
    if (!handle) return 1;

    int result = dicta_task_process(handle, 42);
    if (result != 0) {
        const char* err = dicta_task_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    dicta_task_free(handle);
    return 0;
}
----
...
[source,bash]
----
gcc -o example example.c -ldicta_task -L./zig-out/lib
----
...
[source,rust]
----
#[link(name = "dicta_task")]
extern "C" {
    fn dicta_task_init() -> *mut std::ffi::c_void;
    fn dicta_task_free(handle: *mut std::ffi::c_void);
    fn dicta_task_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = dicta_task_init();
        assert!(!handle.is_null());

        let result = dicta_task_process(handle, 42);
        assert_eq!(result, 0);
...
[source,julia]
----
const libdicta_task = "libdicta_task"

function init()
    handle = ccall((:dicta_task_init, libdicta_task), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:dicta_task_process, libdicta_task), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:dicta_task_free, libdicta_task), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/dicta_task.h
   ```
